...
void liberaListaEncadeada(tElelementoDaListaEncadeada* referenciaElementoDaListaEncadeada){
	if(referenciaElementoDaListaEncadeada != NULL){	//Se o ponteiro estiver apontando pra alguem
		...
		while( *referenciaElementoDaListaEncadeada != NULL){	//Enquanto eu tenho conteudo na lista
			...
			*referenciaElementoDaListaEncadeada = referenciaElementoDaListaEncadeada->proximo;	//Aqui não tem problema eu usar a referencia pra percorrer a lista pois estarei eliminando toda a lista.
			free(noh);
		}
...
void adicionaElementoFinal(tElementoDaListaEncadeada* referenciaElementoDalistaEncadeada, taluno* rap){
	...
	if( referenciaElementoDaListaEncadeada == NULL ){
		...
		noh = (tElementoDaListaEncadeada*)malloc(sizeof(tElementoDaListaEncadeada));	//crio um ponteiro para um novo elemento
		...
		if( noh == NULL){
			...
			if( *referenciaElementoDaListaEncadeada == NULL ){
				...
				*referenciaElementoDaListaEncadeada = noh;		//caso seja, vai apontar pro primeiro elemento(que eu criei: noh)
			}else{
...
int tamanhoListaEncadeada(tElementoDaListaEncadeada* referenciaElementoDaListaEncadeada){
	...
	int contador = 0;
	tElementoDaListaEncadeada *noh;
	noh = *referenciaElementoDaListaEncadeada;	/*Que conteudo de referenciaElementoDaListaEncadeada? proximo ou info? O conteudo desse ponteiro é um outro ponteiro. */
	while( noh != NULL ){
		contador++;
		noh = noh->proximo;
	}
